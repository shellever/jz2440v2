...
#define PFX "s3c2410-wm8976-superlp: "
...
/* wm8976 Register bits */
#define wm8976_ADDR 0x14

#define wm8976_REG_DATA0 (wm8976_ADDR + 0)
#define wm8976_REG_STATUS (wm8976_ADDR + 2)
...
#define AUDIO_NAME_VERBOSE "wm8976 audio driver"
...
static void wm8976_l3_address(u8 data)
{
    int i;
    unsigned long flags;
...
static void wm8976_l3_data(u8 data)
{
    int i;
    unsigned long flags;
                ...
                unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case SOUND_MIXER_INFO:
        {
            ...
            strncpy(info.name,"Philips wm8976", sizeof(info.name));
        ...
        case SOUND_MIXER_WRITE_VOLUME:
            ...
            wm8976_volume = (((val & 0xff) + 1) * 63) / 100;
            wm8976_write_reg(52, (1<<8)|wm8976_volume);
            wm8976_write_reg(53, (1<<8)|wm8976_volume);
            //wm8976_l3_address(wm8976_REG_DATA0);
            //wm8976_l3_data(wm8976_volume);
            break;

        case SOUND_MIXER_READ_VOLUME:
            val = (wm8976_volume * 100) / 63;
            return put_user(val, (long *) arg);
        ...
        case SOUND_MIXER_WRITE_IGAIN:
            ...
            //wm8976_l3_address(wm8976_REG_DATA0);
            //wm8976_l3_data(EXTADDR(EXT0));
            //wm8976_l3_data(EXTDATA(EXT0_CH1_GAIN(mixer_igain)));
            break;
...
static void init_wm8976(void)
{
    ...
    // WM8976G.pdf p78, RESETTING THE CHIP
    // 0x00 - software reset
    wm8976_write_reg(0x00, 0x00);   // a write of any value

    // WM8976G.pdf p80, RECOMMENDED POWER UP/DOWN SEQUENCE
    // 0x03 - power manager 3
    wm8976_write_reg(0x03, 0x6F);   // enable LOUT2EN, ROUT2EN, RMIXEN, LMIXEN, DACENR, DACENL
    // 0x01 - power manager 1
    wm8976_write_reg(0x01, 0x1F);   // enable BIASEN, BUFIOEN, VMIDSEL=11b
    // 0x02 - power manager 2
    wm8976_write_reg(0x02, 0x185);  // enable ROUT1EN, LOUT1EN, INPPGAENL, ADCENL

    // 0x06 - clock gen ctrl
    wm8976_write_reg(0x06, 0x00);   // MCLKDIV = 000, SYSCLK=MCLK
    // 0x04 - audio interface
    wm8976_write_reg(0x04, 0x10);   // WL = 00, 16bit; FMT = 01, Left Justified
    // 0x2B - beep control
    wm8976_write_reg(0x2B, 0x10);   // BTL OUTPUT
    // 0x09 - jack detect control
    wm8976_write_reg(0x09, 0x50);   // enable Jack Detection  
    // 0x0D - jack detect control
    wm8976_write_reg(0x0D, 0x21);   // output switching (jack detect), 1 -> OUT2_EN_1, 0 -> OUT1_EN_0
    // 0x07 - additional ctrl
    wm8976_write_reg(0x07, 0x01);   // enable slow clock, SR = 48kHz
}
...
static int s3c2410iis_probe(struct device *dev) 
{
    ...
    init_wm8976();  // init wm8976 here

    output_stream.dma_ch = DMACH_I2S_OUT;
    if (audio_init_dma(&output_stream, "wm8976 out")) {
    ...
    if (audio_init_dma(&input_stream, "wm8976 in")) {
...
static int __init s3c2410_wm8976_init(void)
{
...
static void __exit s3c2410_wm8976_exit(void)
{
    driver_unregister(&s3c2410iis_driver);
}


module_init(s3c2410_wm8976_init);
module_exit(s3c2410_wm8976_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("superlp<xinshengtaier@eyou.com>");
MODULE_DESCRIPTION("S3C2410 wm8976 sound driver");
