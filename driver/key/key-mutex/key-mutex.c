...
#define CHAR_DEV_NAME "key-mutex"
#define CLASS_NAME    "key"
#define DEVICE_NAME   "key-mutex"


static struct class *key_mutex_class;
static struct class_device *key_mutex_class_device;
...
static struct fasync_struct *key_mutex_queue;
...
static irqreturn_t key_isr(int irq, void *dev_id)
{
    ...
    kill_fasync(&key_mutex_queue, SIGIO, POLL_IN);    // send fasync msg

    return IRQ_HANDLED;
}


static int key_mutex_open(struct inode *inode, struct file *file)
{
    int i, ret;
...
static ssize_t key_mutex_read(struct file *file, char __user *buff, size_t size, loff_t *ppos)
{
    int ret;
...
static unsigned int key_mutex_poll(struct file *file, poll_table *wait)
{
    unsigned int mask = 0;
...
static int key_mutex_fasync(int fd, struct file *file, int on)
{
    printk(KERN_DEBUG "enter %s\n", __func__);

    return fasync_helper(fd, file, on, &key_mutex_queue);
}

static int key_mutex_release(struct inode *inode, struct file *file)
{
    int i;
    ...
    up(&key_lock);  // release lock

    return 0;
}

static struct file_operations key_mutex_fops = {
    .owner = THIS_MODULE,
    .open = key_mutex_open,
    .read = key_mutex_read,
    .poll = key_mutex_poll,
    .fasync = key_mutex_fasync,
    .release = key_mutex_release,
};


static int __init key_mutex_init(void)
{
    printk(KERN_DEBUG "enter %s\n", __func__);

    major = register_chrdev(0, CHAR_DEV_NAME, &key_mutex_fops);
    if (major < 0) {
    ...
    // /sys/class/led
    key_mutex_class = class_create(THIS_MODULE, CLASS_NAME);
    if (IS_ERR(key_mutex_class)) {
        printk(KERN_ERR "class create failed\n");
        goto fail_class_create;
    }
    // /sys/class/led/led-simple --mdev(busybox)--> /dev/led-simple
    key_mutex_class_device = class_device_create(key_mutex_class, NULL, MKDEV(major, 0), NULL, DEVICE_NAME);
    if (IS_ERR(key_mutex_class_device)) {
...
static void __exit key_mutex_exit(void)
{
    printk(KERN_DEBUG "enter %s\n", __func__);

    class_device_unregister(key_mutex_class_device);
    class_destroy(key_mutex_class);
...
module_init(key_mutex_init);
module_exit(key_mutex_exit);

MODULE_AUTHOR("Shell Ever <shellever@163.com>");
MODULE_DESCRIPTION("jz2440v2 key mutex driver");
MODULE_LICENSE("GPL");
