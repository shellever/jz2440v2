...
#define CHAR_DEV_NAME "key-interrupt"
#define CLASS_NAME    "key"
#define DEVICE_NAME   "key-interrupt"


static struct class *key_interrupt_class;
static struct class_device *key_interrupt_class_device;
...
static volatile int ev_press = 0;   // press event
...
static ssize_t key_interrupt_read(struct file *file, char __user *buff, size_t size, loff_t *ppos)
{
    int ret;
    ...
    wait_event_interruptible(key_waitqueue, ev_press);  // will sleep here
...
static int __init key_interrupt_init(void)
{
    ...
    major = register_chrdev(0, CHAR_DEV_NAME, &key_interrupt_fops);
    if (major < 0) {
    ...
    // /sys/class/led
    key_interrupt_class = class_create(THIS_MODULE, CLASS_NAME);
    if (IS_ERR(key_interrupt_class)) {
        printk(KERN_ERR "class create failed\n");
        goto fail_class_create;
    }
    // /sys/class/led/led-simple --mdev(busybox)--> /dev/led-simple
    key_interrupt_class_device = class_device_create(key_interrupt_class, NULL, MKDEV(major, 0), NULL, DEVICE_NAME);
    if (IS_ERR(key_interrupt_class_device)) {
...
static void __exit key_interrupt_exit(void)
{
    ...
    class_device_unregister(key_interrupt_class_device);
    class_destroy(key_interrupt_class);
...
MODULE_DESCRIPTION("jz2440v2 key interrupt driver");
MODULE_LICENSE("GPL");
